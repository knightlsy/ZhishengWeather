...
class WidgetLayoutTest {
    ...
    fun widgetDetailsAreSizedPerLayoutInsteadOfBeingEllipsized() {
        ...
        assertTrue(source.contains("layout == R.layout.widget_large && snap.windText.isNotBlank()"))
        // v0.0.4：2x2 主动舍弃更新时间——布局不再保留恒 GONE 的 w_upd 占位，
        // Provider 仅对非 small 档位写 w_upd（原断言检查 GONE 分支，随死控件移除更新）
        assertTrue(source.contains("if (layout != R.layout.widget_small) v.setViewVisibility(R.id.w_upd, View.VISIBLE)"))
