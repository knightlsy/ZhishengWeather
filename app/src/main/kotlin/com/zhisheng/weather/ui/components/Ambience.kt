...
fun WeatherAmbience(
    ...
    // 主题色提前取值（DrawScope 扩展非 composable 上下文，v0.0.5 浅色适配）
    val palette = LocalZhishengPalette.current
    val rainMint = palette.mint
    val snowText = palette.text
    val fogDot = if (palette.isLight) palette.text else Color.White
    val stormCyan = palette.cyan
    val flashColor = if (palette.isLight) palette.text else Color.White
    ...
    Canvas(
        ...
        when (kind) {
            AmbienceKind.RAIN -> drawDataRain(t, f, motion, particles, density.density, monoTypeface, rainMint)
            AmbienceKind.SNOW -> drawSnow(t, f, motion, particles, snowText)
            AmbienceKind.FOG -> drawFogNoise(t, f, fogPath, fogDot)
            AmbienceKind.STORM -> drawStormScan(t, f, vivid, stormCyan, flashColor)
...
// —— 飘雪：缓慢下落 + 正弦横向漂移 ——
private fun DrawScope.drawSnow(t: Float, f: Float, motion: Float, particles: List<Particle>, textColor: Color) {
    val colW = size.width / 22f
    particles.forEach { p ->
        ...
        drawCircle(
            color = textColor.copy(alpha = (0.085f * f).coerceIn(0f, 1f)),
            radius = r,
            center = Offset(x, y),
        )
    }
...
private fun DrawScope.drawFogNoise(t: Float, f: Float, fogPath: Path?, dotColor: Color) {
    if (fogPath == null) return
    // 呼吸周期约 7 秒
    val breathe = (sin((t * 0.9f).toDouble()).toFloat() + 1f) / 2f
    val alpha = (0.03f + 0.045f * breathe) * f
    drawPath(fogPath, dotColor.copy(alpha = alpha.coerceIn(0f, 1f)))
}
...
private fun DrawScope.drawStormScan(t: Float, f: Float, vivid: Boolean, cyanColor: Color, flashColor: Color) {
    ...
    if (local < 1.5f) {
        ...
        // 主线 + 上方拖尾
        drawRect(
            color = cyanColor.copy(alpha = (0.05f * f).coerceIn(0f, 1f)),
            topLeft = Offset(0f, y - bandH),
            size = Size(size.width, bandH),
        )
        drawLine(
            color = cyanColor.copy(alpha = (0.14f * f).coerceIn(0f, 1f)),
            start = Offset(0f, y),
            end = Offset(size.width, y),
            strokeWidth = 1.4f,
        )
    }
    // 闪白：周期末尾两次短促脉冲（浅色纸面上改闪墨色，白纸闪白不可见，v0.0.5）
    val flashAt = period - 0.55f
    if (local > flashAt) {
        val d = local - flashAt
        val pulse = if (d < 0.08f) 1f else if (d in 0.18f..0.24f) 0.6f else 0f
        if (pulse > 0f) {
            drawRect(color = flashColor.copy(alpha = (0.030f * f * pulse).coerceIn(0f, 1f)))
        }
    }
}
